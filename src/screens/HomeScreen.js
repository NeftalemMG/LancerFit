...
import { awardQuestPoints } from "../services/questApi";
...
function QuestRow({ q, onPress, onClaim }) {
  ...
  return (
    <PressScale onPress={onPress} style={{ marginBottom: 10 }}>
      <Card
        style={[styles.quest, styles.questGold, q.claimed && { opacity: 0.6 }]}
      >
        <View style={[styles.qicon, q.gold ? styles.qiconGold : null]}>
          <QuestGlyph
            name={q.icon}
            color={questIconColor[q.icon] || colors.blue2}
            size={22}
          />
        </View>
        <View style={{ flex: 1, minWidth: 0 }}>
          ...
          <View style={{ marginTop: 9 }}>
            <ProgressBar
              pct={(q.cur / q.max) * 100}
              height={5}
              fillColor={done ? colors.green : colors.gold}
            />
          </View>
        </View>
        {done && !q.claimed ? (
...
export default function HomeScreen({ navigation }) {
  const { player, quests, bumpQuest, claimQuest, checkIn, refreshMe } =
    useApp();
  const {
    steps,
    weekVals,
  ...
  const claimQuestPoints = async (id) => {
    try {
      if (id) {
        const claim = await awardQuestPoints(id);
        claimQuest(id);
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.xpMeta}>
          <Text style={styles.xpMetaText}>
            <Text style={styles.xpMetaBold}>{fmt(xp)}</Text> / {fmt(xpMax)} XP
          </Text>
          <Text style={styles.xpMetaText}>
            <Text style={styles.xpMetaBold}>
              {fmt(Math.max(0, xpMax - xp))}
            </Text>{" "}
            to next
          </Text>
        </View>
      ...
      <Card style={[styles.checkin, player.checkedIn && styles.checkinDone]}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          ...
          <View style={styles.openRow}>
            <View
              style={[
                styles.dot,
                { backgroundColor: nearGym ? colors.green : colors.text3 },
              ]}
            />
        ...
        <PressScale
          onPress={doGymCheckIn}
          disabled={checking || player.checkedIn}
        >
          <LinearGradient
            colors={
              player.checkedIn
          ...
          >
            {player.checkedIn ? (
              <CheckIcon size={24} color="#fff" strokeWidth={2.6} />
            ) : (
              <ScanIcon
                size={22}
                color={nearGym ? "#000000" : "#fff"}
                strokeWidth={2.2}
              />
            )}
            <Text
              style={[
                styles.scanLbl,
                { color: nearGym && !player.checkedIn ? "#000" : "#fff" },
              ]}
            >
              {player.checkedIn ? "Done" : "Check\nin"}
            </Text>
          </LinearGradient>
        </PressScale>
      </Card>
...
const styles = StyleSheet.create({
  ...
  ciIcoDone: {
    backgroundColor: "rgba(79,181,135,0.2)",
    borderColor: colors.greenLine,
  },
  ciTitle: {
    fontFamily: disp.bold,
  ...
  scanBtn: {
    ...
    justifyContent: "center",
    gap: 3,
    ...shadow.accent("rgba(47,123,196,0.9)"),
  },
  scanLbl: {
    fontFamily: disp.bold,
    fontSize: 9,
